...
parser.add_argument('--mouseID', type=str, nargs='?', default='mouse678912', help='mouseID in form mouse#####')
...
def copy_files(src_path, dst_path):
    ...
    for item in os.listdir(src_path):
        ...
        if os.path.isfile(item_path):
            ...
            if os.path.exists(os.path.join(dst_path, item)):
                print(f'Skipping {item_path}')
                continue

            # copy file to destination path if smaller than 5 GB
            if os.path.getsize(item_path) < MAX_FILE_SIZE:
                print(f'Copying: {item_path}')
                shutil.copy(item_path, dst_path)
            else:
                # create an empty file with the same name in the destination path
                empty_file_path = os.path.join(dst_path, item)
