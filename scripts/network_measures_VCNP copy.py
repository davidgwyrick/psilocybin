...
DataDir = '/data/projects/zap-n-zip/data/VCNP/'
SaveDir = '/data/projects/zap-n-zip/results/ccm/VCNP/'
...
if __name__ == '__main__':
    cmap = np.concatenate((sns.color_palette('Blues',4),sns.color_palette('Greys',4)))
...
#     SaveDir = os.path.join(TempDir,folder)
    ...
    plt.suptitle('Weighted in/out degree magnitudes per area; connections exist for FC > 0.1',y=0.935)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[0],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[1],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[2],legend=False)
    ...
    fig, axes = plt.subplots(3,1,figsize=(10,12))
    plt.suptitle('Weighted in/out degree magnitudes per area; connections exist for FC > 0',y=0.935)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[0],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[1],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='area',y='degree',hue='epoch',palette=cmap,ax=axes[2],legend=False)
    ...
    areas_of_interest = np.unique(areas)
    position_list = []
    tmp = np.arange(nCond)
    xticks = []
    for ii in range(len(areas_of_interest)):
    ...
    for a in areas_of_interest:
        ...
        n = len(indy)
        fig, axes = plt.subplots(3,1,figsize=(10,12))
        plt.suptitle(f'Source area {a}, n = {n}; connections exist for FC > 0.1',y=0.94)

        tmp_list = []
        counter = 0
        for a2 in areas_of_interest:
            ...
            for ii in range(nCond):
                ...
                tmp_list.append((np.repeat(filename_list[ii],n1),np.repeat(a,n1),np.repeat(a2,n1),np.repeat('out',n1),in_degree))
        ...
        sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='target_area',order=areas_of_interest,y='degree',hue='epoch',palette=list(cmap),ax=axes[0],legend=False)
        sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='target_area',order=areas_of_interest,y='degree',hue='epoch',palette=list(cmap),ax=axes[1],legend=False)
        sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='target_area',order=areas_of_interest,y='degree',hue='epoch',palette=list(cmap),ax=axes[2],legend=False)
    ...
    fig, axes = plt.subplots(3,1,figsize=(10,12))
    sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='area',order=areas_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[0],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='area',order=areas_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[1],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='area',order=areas_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[2],legend=False)
    ...
    for jj in range(nCond):
        ...
        for ii, (g_type, g) in enumerate(zip(['undirected_weight','undirected_adjmat','directed_weight'],[UG_weight,UG_adjmat,DG_weight])):
            ...
            if g_type == 'directed_weight':
                ...
                plt.suptitle(f'# of neurons per group per community: {filename_list[jj]}, Graph type: {g_type}')
    ...
    try:
        ...
        prs.save(join(PlotDir,f'FCF_figs5_{session_id}.pptx'))
        exit()
