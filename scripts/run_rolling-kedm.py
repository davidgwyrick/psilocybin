...
parser.add_argument('--rec_name',type=str, default='aw_psi_2024-04-11_10-37-43',
                    help='experiment to perform analysis on')

parser.add_argument('--time_bin_ms',type=int, default=100,
                    help='Time bin width')

parser.add_argument('--tWindow_width_min',type=int, default=10,
...
if __name__ == '__main__':
    ...
    TempDir = os.path.join(ServDir,'results','kedm_rolling_PCA',mID,rec_name)
    ...
    if (drug_type == 'ketanserin+psilocybin') | (mID == 'mouse728449'):
    ...
    np.savez(os.path.join(SaveDir,f'time_windows.npz'),time_window_list=time_window_list,epoch_list=epoch_list,neuron_indices=neuron_indices,
             running_moments=running_moments,pupil_moments=pupil_moments)


    #Read in preprocessed data
    DataDir = join(ServDir,'data','PCA_area_level',mID,rec_name)
    print('Area-level')
    data = np.load(join(DataDir,f'pca_{mID}_{rec_name}.npz'))
    data_pc = data['data_pc']
    groups_pc = data['groups_pc']
    ts_all = data['ts']
    ticks = data['ticks']
    labels = data['labels']
    ...
    for ii, tW in enumerate(time_window_list):
        ...
        data = data_pc[indy]; ts = ts_all[indy]
        T, N = data.shape
        X = data

        group_comm_list = []
        for grp in np.unique(groups_pc):
            indy = np.where(groups_pc == grp)[0]
            group_comm_list.append(frozenset(indy))
        ...
        # if area_info:
        #     np.savez(join(SaveDir,f'rolling-FC_{epoch_list[ii]}.npz'),FCF_takens=FCF_takens,correlation=correlation,FCF_optimal=FCF_optimal,complexity=complexity,mID=mID,rec_name=rec_name,tWindow=tW,epoch=epoch_list[ii],areas_pop=areas_pop,groups_pop=groups_pop,area_list2=area_list2)
        # else:
