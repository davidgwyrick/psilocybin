...
parser.add_argument('--tWindow_width_s',type=int, default=10,
                    help='Time window over which to calculate FC')

parser.add_argument('--tWindow_shift_s',type=float, default=2,
...
parser.add_argument('--parallel',type=int, default=1,
                    help='Parallize calculation over windows')
...
def calculate_phase_synchronization(spike_time_dict, tWindow, dt, method = 'continuous'):
    ...
    for i, spk_times in spike_time_dict.items():
        if len(spk_times) <= 2:
            ...
            phase = get_phase_spikes(spk_times, tWindow, dt, method)
    ...
    for i, j in it.combinations(range(N),2):
        ...
        phase_synch[i,j] = np.sqrt(np.square(np.mean(np.cos(delta_phi))) + 
                                    np.square(np.mean(np.sin(delta_phi))))
        phase_synch[j,i] = phase_synch[i,j]

    return phase_synch #, phase_list

@ray.remote
def calculate_phase_synchronization_ray(spike_time_dict, tWindow, dt, pba = None, method = 'continuous'):
    phase_synch = calculate_phase_synchronization(spike_time_dict, tWindow, dt, method = 'continuous')
...
if __name__ == '__main__':
    ...
    tWindow_shift_s = args.tWindow_shift_s
    ...
    tW_starts = np.arange(open_ephys_start,open_ephys_end-tWindow_width_s,tWindow_shift_s)
    tW_ends = tW_starts + tWindow_width_s
    time_window_array = np.array((tW_starts,tW_ends)).T
    time_window_list = time_window_array.tolist()
    time_window_centers = time_window_array[:,0] + tWindow_width_s/2
    nWindows = len(time_window_list)
    print(f'{nWindows} windows, {tWindow_width_s} seconds long, separeted by {tWindow_shift_s} seconds')
    ...
    #Read in spiking data for area definitions
    # tW = time_window_list[0] #[open_ephys_start,open_ephys_end]
    ...
    ## Parallelize ------------------------------------------
    if parallel:
        print(f'Parallel processing over {nWindows} windows')
        ray.init(num_cpus=26,include_dashboard=True, dashboard_port=5432)
        processes = []
        pb = util.ProgressBar(nWindows); actor = pb.actor
    else:
        print(f'Serial processing over {nWindows} windows')
    ...
    for ii, tW in enumerate(time_window_list):
        ...
        #Get spike times for this window
        spike_time_dict_epoch = {}
        for key in spike_time_dict.keys():
            spike_time_dict_epoch[key] = spike_time_dict[key][np.where((spike_time_dict[key] >= tW[0]) & (spike_time_dict[key] < tW[1]))]

        if parallel:
            #Add to ray
            processes.append(calculate_phase_synchronization_ray.remote(spike_time_dict_epoch, tW, dt,pba=actor))
        else:
            synchrony[ii] = calculate_phase_synchronization(spike_time_dict_epoch, tW, dt)

    if parallel:
        #And then print progress bar until done
        pb.print_until_done()

        #Initiate parallel processing & get results
        synchrony = np.array(ray.get(processes))
    ...
    #         synchrony[ii] = result[0]
    #         phase_list = result[1]
    #         phase_list_list.append(phase_list)

    tE = (time.perf_counter() - t0)/60
    print('\tProcessing completed in {:.2f} mins'.format(tE))
    ## End Parallelize ------------------------------------------

    ## Use phase synchronization matrix to calculate global synchrony
    # Calculate correlation as well
    ...
    for ii, tW in enumerate(time_window_list):
        ...
        #Calculate global synchrony for all pairs of neurons
        tmp_synchrony = synchrony[ii].copy()
    ...
    ## Plot global synchrony as a function of behavior ----------------------------------------
