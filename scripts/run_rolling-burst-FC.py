...
parser.add_argument('--tWindow_width_s',type=int, default=150,
...
if __name__ == '__main__':
    ...
    data_list, ts_list, _, plot_tuple, _ = util.bin_spiking_data(probe_unit_data, [tW_full], time_bin=time_bin,fr_thresh=fr_thresh)
    ...
    #Concatenate spiking data & burst data
    X_all = np.concatenate((data,burst_rate.T),axis=1)
    ...
    for ii, tW in enumerate(time_window_list):
        ...
        #Remove neurons with no spikes or bursts
        neural_indices = np.where(np.nansum(X,axis=0) > 0)[0]
        X = X[:,neural_indices]
        n = len(neural_indices)

        ijs = list(it.combinations(range(n),2))
        combos = list(it.combinations(neural_indices,2))
        ...
        correlation_plot = np.zeros((nN*2,nN*2))
        correlation_plot[np.ix_(neural_indices,neural_indices)] = correlation
        ...
        fig = usrplt.plot_CCM_results(correlation_plot,FCF_plot,DIR_plot,CPX_plot,vmax_fcf=None,dir_clims=None,vmax_takens=None,title=epoch_list[ii])#,ticks=ticks,labels=labels,boundaries=boundaries)
