    ...
    for g in groups_to_iterate:
        ...
        for iW, (epoch, tW) in enumerate(zip(epoch_names,epoch_windows)):
            ...
            for iA, A in enumerate([S, D]):
                F_thresh = args.F_thresh
                # fpath = join(SaveDir,f'goodness-of-fit_A-{ava_strs[iA]}_{g}_{epoch}.npz')
                # if os.path.exists(fpath):
                #     print(f'{fpath} exists; skipping')
                #     r = np.load(fpath)

                #     F = r['F']; KS = r['KS']; tau = r['tau']; plr = r['plr']; nAv = r['nAv']
                #     i,j = r['i'],r['j']
                #     uniq_DS = np.unique(A)
                ...
                # plr2 = np.full((nCombinations),np.nan)
                # ijs2 = np.full((nCombinations,2),np.nan)
                # xmm2 = np.full((nCombinations,2),np.nan)
                # index = 0
                # for (i,j), (xmin,xmax) in zip(uniq_ijs,uniq_xmm):
                #     plr2[index] = np.log10(xmax/xmin)
                #     ijs2[index] = [i,j]
                #     xmm2[index] = [xmin,xmax]
                #     index += 1
                # arg_indy = np.argsort(plr2)[::-1]
                # uniq_ijs = ijs2[arg_indy]
                # uniq_xmm = xmm2[arg_indy]
                # candidate_fit_found = False
                # for (i,j), (xmin,xmax) in zip(uniq_ijs,uniq_xmm):
                #     d = np.log10(xmax/xmin)
                #     i = int(i); j = int(j)
                #     print(f'({i},{j}): [{xmin},{xmax}], {d:.2f} decades')
                #     plr[i,j] = np.log10(xmax/xmin)
                #     nAv[i,j] = len((S >= xmin) & (S <= xmax))
                #     F[i,j], KS[i,j], tau[i,j] = goodness_of_fit(A, xmin, xmax, pl_distr)
                    
                #     if F[i,j] > F_thresh:
                #         print(f'\t\t\tGood fit: F = {F[i,j]:.2f}; KS = {KS[i,j]:.2e}; tau = {tau[i,j]:.2f}')
                #         candidate_fit_found = True
                #         break

                # if not candidate_fit_found:
