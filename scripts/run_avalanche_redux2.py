...
parser.add_argument('--F_thresh',type=float, default=0.8,
...
def goodness_of_fit(data, xmin, xmax, distribution='power_law',pba=None): 
    ...
    if distribution == 'truncated_power_law':
        ...
        pl_model = pl.Power_Law(xmin=xmin, parameters=params)
    ...
    resampled_points, actual_CDF_resampled = resample_cdf(bins, actual_CDF, 100)
...
if __name__ == '__main__':
    ...
    TempDir = os.path.join(ServDir,'results','avalanches_1225',mID,rec_name)
    ...
    if drug_type in ['saline', 'psilocybin','ketanserin+psilocybin','urethane+psilocybin']:
        ...
        epoch_windows.append([injection_time_windows[1,1],open_ephys_end]); epoch_names.append(f'post-{injection_types[1]}-inj')
    ...
    for g in groups_to_iterate:
        ...
        for iW, (epoch, tW) in enumerate(zip(epoch_names,epoch_windows)):
            ...
            best_fit_pl = []
            for iA, A_list in enumerate([(S,D), (D,S)]):
                F_thresh = args.F_thresh
                A = A_list[0]
                A_complement = A_list[1]
                ...
                F =  np.full((2,nDS,nDS),np.nan)
                KS = np.full((2,nDS,nDS),np.nan)
                tau = np.full((2,nDS,nDS),np.nan)
                ...
                if parallel:
                    ...
                    pb = util.ProgressBar(nCombinations*2); actor = pb.actor

                processes = []; processes_complement = []; process_ijs = []
                for (i,j), (xmin,xmax) in zip(uniq_ijs,uniq_xmm):
                    plr[i,j] = np.log10(xmax/xmin)
                    nAv[i,j] = len((A >= xmin) & (A <= xmax))
                    indy = np.where((A >= xmin) & (A <= xmax))[0]
                    A_sub = A[indy]
                    AC_sub = A_complement[indy]
                    xmin_AC = np.nanmin(AC_sub); xmax_AC = np.nanmax(AC_sub)
                    #Calculate goodness of fit
                    if parallel:
                        processes.append(goodness_of_fit_p.remote(A, xmin, xmax, pl_distr, actor))
                        processes.append(goodness_of_fit_p.remote(AC_sub, xmin_AC, xmax_AC, pl_distr, actor))
                        process_ijs.append((0,i,j))
                        process_ijs.append((1,i,j))
                    else:
                ...
                if parallel:
                    ...
                    #Extract results
                    for (k,i,j), r in zip(process_ijs,results):
                        F[k,i,j], KS[k,i,j], tau[k,i,j] = r
                ...
                np.savez(join(SaveDir,f'goodness-of-fit_A-{ava_strs[iA]}_{g}_{epoch}.npz'),D=D,S=S,loc=loc,F=F,KS=KS,tau=tau,plr=plr,nAv=nAv)#,i=i,j=j)
    ...
    #     tmp_df = DCC_df[(DCC_df['group'] == g)]
    #     tau0 = tmp_df.loc[tmp_df['epoch'] == epoch_names[0]]['diff'].values[0]
    #     tau2 = tmp_df.loc[tmp_df['epoch'] == epoch_names[2]]['diff'].values[0]
    #     tmp_list.append([mID,rec_name,g,epoch_names[0],epoch_names[2],tau0,tau2,tau2-tau0])
    # DCC_diff_df = pd.DataFrame(tmp_list,columns=['mID','rec_name','group','epoch0','epoch2','DCC0','DCC2','diff'])
    # DCC_diff_df = DCC_diff_df.astype({'DCC0':float,'DCC2':float,'diff':float})

    # tmp_list = []
    # for g in pl_df['group'].unique():
    #     for a in pl_df['type'].unique():
    #         tmp_df = pl_df[(pl_df['group'] == g) & (pl_df['type'] == a)]
    #         tau0 = tmp_df.loc[tmp_df['epoch'] == epoch_names[0]]['tau'].values[0]
    #         tau2 = tmp_df.loc[tmp_df['epoch'] == epoch_names[2]]['tau'].values[0]
    #         tmp_list.append([mID,rec_name,g,a,epoch_names[0],epoch_names[2],tau0,tau2,tau2-tau0])

    # pl_diff_df = pd.DataFrame(tmp_list,columns=['mID','rec_name','group','type','epoch0','epoch2','tau0','tau2','diff'])
    # pl_diff_df = pl_diff_df.astype({'tau0':float,'tau2':float,'diff':float})

    # cmap2 = np.stack([epoch_cmap[0],epoch_cmap[2]])
    ...
    # ax = axes[0]; ax.set_title('Avalanche size')
    # sns.boxplot(data=pl_df.loc[pl_df.type == 'size'],x='epoch',y='tau',ax=ax,palette=cmap2)
    # # ax.boxplot(pl_df.loc[(pl_df.type == 'size') & (pl_df.epoch == epoch_names[0])]['tau'].values,positions=[0],widths=0.5,palette=epoch_cmap[0])
    # tau0 = pl_diff_df.loc[pl_diff_df.type == 'size']['tau0']
    # tau2 = pl_diff_df.loc[pl_diff_df.type == 'size']['tau2']
    # ax.plot([0,1],[tau0,tau2],'.-',color='k',alpha=0.5)
    # ax.set_ylabel('\u03C4')

    # #Power law exponents for avalanche duration
    # ax = axes[1]; ax.set_title('Avalanche duration')
    # sns.boxplot(data=pl_df.loc[pl_df.type == 'duration'],x='epoch',y='tau',ax=ax,palette=cmap2)
    # ax.set_ylabel('\u03C4_t')
    # tau0 = pl_diff_df.loc[pl_diff_df.type == 'duration']['tau0']
    # tau2 = pl_diff_df.loc[pl_diff_df.type == 'duration']['tau2']
    # ax.plot([0,1],[tau0,tau2],'.-',color='k',alpha=0.5)

    # #Deviation from criticality
    # ax = axes[2]; ax.set_title('Deviation from criticality')
    # sns.boxplot(data=DCC_df,x='epoch',y='diff',ax=ax,palette=cmap2)
    # ax.set_ylabel('DCC')
    # tau0 = DCC_diff_df['DCC0']
    # tau2 = DCC_diff_df['DCC2']
    # ax.plot([0,1],[tau0,tau2],'.-',color='k',alpha=0.5)
    # for ax in axes:
    #     usrplt.adjust_spines(ax)
    ...
    # plt.savefig(join(PlotDir,f'avalanche_boxplot_{g}_{epoch}.png'),dpi=300,bbox_inches='tight',facecolor='w')
    # usrplt.save_fig_to_pptx(fig, prs)
    # plt.close(fig)
