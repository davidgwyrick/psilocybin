...
if __name__ == '__main__':
    ...
    #Define directories
    SaveDir = join(ServDir,'results','behavior')
    ...
    slide.shapes.title.text = f'Locomotion & pupil behavior'
    slide.placeholders[1].text = f'Behavior!'

    #Loop over animals
    for mID, row in exp_table.iterrows():
        rec_name = row.exp_name
        
        if ('iso' in rec_name) | ('urethane' in rec_name) | ('ur' in rec_name):
            continue
    ...
    for indy, fsuffix in zip([np.arange(nNeurons),neuron_indices],['ALL','sub']):
        ...
        for j, tWindow in enumerate(inter_sweep_windows):
            ...
            for key, behav_str in behavior_dict.items():
                ...
                tmp_list.append(np.stack((np.repeat(mID,N),np.repeat(rec_name,N),indy,celltypes[indy], durations[indy], areas[indy],groups[indy],supergroups[indy],np.repeat(behav_str,N),np.repeat(epoch_j,N),FR_j)))
        FR_spont_df = pd.DataFrame(np.hstack(tmp_list).T,columns=['mID','rec_name','neuron_index','celltype','duration','area','group','supergroup','behavior','epoch','FR'])
        FR_spont_df = FR_spont_df.astype({'mID':str,'rec_name':str,'neuron_index': int,'celltype':str,'duration':float,'area':str,'group':str,'supergroup':str,'behavior':str,'epoch':str,'FR':float})
        ...
        for cb in combos_spont:
            ...
            for key, behav_str in behavior_dict.items():
                ...
                tmp_list.append(np.stack((np.repeat(mID,N),np.repeat(rec_name,N),indy,celltypes[indy],durations[indy],areas[indy],groups[indy],supergroups[indy],np.repeat(behav_str,N),np.repeat(epoch_i,N),np.repeat(epoch_j,N),np.repeat(tW_diff,N),FR_i,FR_j,dFR,MI)))
            
        dFR_spont_df = pd.DataFrame(np.hstack(tmp_list).T,columns=['mID','rec_name','neuron_index','celltype','duration','area','group','supergroup','behavior','epoch_i','epoch_j','tW_diff','FR_i','FR_j','dFR','MI'])
        dFR_spont_df = dFR_spont_df.astype({'mID':str,'rec_name':str,'neuron_index':int,'celltype':str,'duration':float,'area':str,'group':str,'supergroup':str,'behavior':str,'epoch_i':str, 'epoch_j':str,'tW_diff':float,'FR_i':float,'FR_j':float,'dFR':float,'MI':float})
        ...
        for cb in combos_evoked:
            ...
            for k, behav_str in behavior_dict.items():
                for l, w_str in enumerate(window_strs):
                    ...
                    tmp_list.append(np.stack((np.repeat(mID,N),np.repeat(rec_name,N),indy,celltypes[indy],durations[indy],areas[indy],groups[indy],supergroups[indy],np.repeat(behav_str,N),np.repeat(w_str,N),np.repeat(epoch_i,N),np.repeat(epoch_j,N),np.repeat(tW_diff,N),FR_i_sub,FR_j_sub,dFR,MI)))
                        
        dFR_evoked_df = pd.DataFrame(np.hstack(tmp_list).T,columns=['mID','rec_name','neuron_index','celltype','duration','area','group','supergroup','behavior','window','epoch_i','epoch_j','tW_diff','FR_i','FR_j','dFR','MI'])
        dFR_evoked_df = dFR_evoked_df.astype({'mID':str,'rec_name':str,'neuron_index':int,'celltype':str,'duration':float,'area':str,'group':str,'supergroup':str,'behavior':str,'window':str,'epoch_i':str, 'epoch_j':str,'tW_diff':float,'FR_i':float,'FR_j':float,'dFR':float,'MI':float})
