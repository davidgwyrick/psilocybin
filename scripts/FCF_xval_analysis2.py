...
if __name__ == '__main__':
   ...
   #Find folder with results inside
    ...
    fcf_moments = np.zeros((nCond,nKfold,len(labels_group)+2,6))
    sig_elements = np.zeros((nCond,nKfold,len(labels_group)+2))
    for ll, g in enumerate(labels_group):
        indy = np.where(groups_sub == g)
        #Calculate mean and percentile of FCF per epoch/kfold
        for ii in range(nCond):
            for iK in range(nKfold):
                tmp_array = FCF_optimal[ii,iK][:,indy]
                fcf_moments[ii,iK,ll,0] = np.nanmean(tmp_array)
                fcf_moments[ii,iK,ll,1] = np.nanpercentile(tmp_array,75)
                fcf_moments[ii,iK,ll,2] = np.nanpercentile(np.nanmean(tmp_array,axis=0),75)

                tmp_array = FCF_optimal_kf_mask[ii,iK][:,indy]
                tmp = np.copy(tmp_array).ravel()
                mask = tmp > 0.1

                sig_elements[ii,iK,ll] = np.sum(mask)/len(mask) 
                fcf_moments[ii,iK,ll,3] = np.nanmean(tmp_array)
                fcf_moments[ii,iK,ll,4] = np.nanpercentile(tmp_array,75)
                fcf_moments[ii,iK,ll,5] = np.nanpercentile(np.nanmean(tmp_array,axis=0),75)

    supergroups_sub = supergroups[neuron_indices]        
    for ll, sg in zip([-2,-1],['CTX','TH']):
        indy = np.where(supergroups_sub == sg)
        #Calculate mean and percentile of FCF per epoch/kfold
        for ii in range(nCond):
            for iK in range(nKfold):
                tmp_array = FCF_optimal[ii,iK][:,indy]
    ...
    for ll, g in enumerate(tmp_labels):
        for x_array, xlabel in zip((pupil_moments,running_moments),('pupil radius (pixels)','running speed (cm/s)')):
            for jj, ylabel in enumerate(['Mean FC','75th percentile FC','75th percentile FC_j']):
                fig, axes = plt.subplots(1,2,figsize=(10,5))
                plt.suptitle(f'{mID}, {g}; Trends in {ylabel}')
                for ii, data in enumerate(data_list):
                    for iK in range(nKfold):
                        axes[0].scatter(x_array[ii,iK,0],fcf_moments[ii,iK,ll,jj],marker='.',facecolors="None",edgecolors=cmap[ii])#,linewidths=2)#,label=fname)
                        axes[1].scatter(x_array[ii,iK,0],fcf_moments[ii,iK,ll,jj+3],marker='.',facecolors="None",edgecolors=cmap[ii])#,linewidths=2)#,label=fname)

                    axes[0].plot(np.nanmean(x_array[ii,:,0]),np.nanmean(fcf_moments[ii,:,ll,jj]),'o',color=cmap[ii])#,label=fname)
                    axes[1].plot(np.nanmean(x_array[ii,:,0]),np.nanmean(fcf_moments[ii,:,ll,jj+3]),'o',color=cmap[ii])#,label=fname)

                x = x_array[:,:,0].ravel()
                y = fcf_moments[:,:,ll,jj].ravel()
                r = st.linregress(x,y)
    ...
    for ll, g in enumerate(labels_group):
        ...
        lw = 1.5; zo = 0; ls = '-'
        plt.suptitle(f'{mID}; FCF_j for group {g}; {len(indy)} neurons')
        fcf_j_sal = []
        fcf_j_psi = []
        for ii in range(nCond):
            for iK in range(nKfold):
        ...
        fig, ax = plt.subplots(figsize=(8,8))
        lw = 1.5; zo = 0; ls = '-'
        plt.suptitle(f'{mID}; FCF_j, mean over kfolds, for group {g}; {len(indy)} neurons')
        fcf_j_sal = []
        fcf_j_psi = []
        for ii in range(nCond):
        ...
        fig, ax = plt.subplots(figsize=(8,8))
        lw = 1.5; zo = 0; ls = '-'
        plt.suptitle(f'{mID}; masked FCF_j for group {g}; {len(indy)} neurons')
        fcf_j_sal = []
        fcf_j_psi = []
        for ii in range(nCond):
            for iK in range(nKfold):
        ...
        fig, ax = plt.subplots(figsize=(8,8))
        lw = 1.5; zo = 0; ls = '-'
        plt.suptitle(f'{mID}; masked FCF_j, mean over kfolds, for group {g}; {len(indy)} neurons')
        fcf_j_sal = []
        fcf_j_psi = []
        for ii in range(nCond):
    ...
    # prs.save(join(SaveDir,f'FCF_figs4_{rec_name}.pptx'))
    # print('DONE!!!')
    ...
    ##----- Plot actual FCF matrices I guess -----##
    ...
    for jj, fname in enumerate(filename_list):
        ...
        sns.kdeplot(degree_dist,ax=ax_out,color=cmap[jj],label=label_list[jj])
    ...
    ax_out.set_xlim(xmin=0)
    usrplt.adjust_spines(ax_out)
    usrplt.save_fig_to_pptx(fig_out, prs)
    ...
    plt.suptitle('Weighted in/out degree magnitudes per group; connections exist for FC > 0.1',y=0.935)
    ...
    fig, axes = plt.subplots(3,1,figsize=(10,12))
    plt.suptitle('Weighted in/out degree magnitudes per group; connections exist for FC > 0',y=0.935)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[0],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[1],legend=False)
    sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[2],legend=False)
    ...
    for g in groups_of_interest:
        ...
        n = len(indy)
        fig, axes = plt.subplots(3,1,figsize=(10,12))
        plt.suptitle(f'Source group {g}, n = {n}; connections exist for FC > 0.1',y=0.94)
        ...
        for g2 in groups_of_interest:
            ...
            for ii in range(nCond):
                ...
                tmp_mat = FCF_optimal_kf_mask[ii].copy()
                in_degree = np.nansum(tmp_mat[:][:,indy2][:,:,indy],axis=1).ravel(); n1 = len(in_degree)
                out_degree = np.nansum(tmp_mat[:][:,indy][:,:,indy2],axis=2).ravel(); n2 = len(out_degree)
                ...
                tmp_list.append((np.repeat(filename_list[ii],n1),np.repeat(g,n1),np.repeat(g2,n1),np.repeat('out',n1),in_degree))
    ...
    for g in groups_of_interest:
        ...
        n = len(indy)
        fig, axes = plt.subplots(3,1,figsize=(10,12))
        plt.suptitle(f'Source group {g}, n = {n}; connections exist for FC > 0',y=0.94)
        ...
        sns.barplot(data=degree_df.loc[degree_df.type == 'in'],x='target_group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[0],legend=False)
        sns.barplot(data=degree_df.loc[degree_df.type == 'out'],x='target_group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[1],legend=False)
        sns.barplot(data=degree_df.loc[degree_df.type == 'in/out'],x='target_group',order=groups_of_interest,y='degree',hue='epoch',palette=cmap,ax=axes[2],legend=False)
    ...
    for jj in range(nCond):
        ...
        for iK in range(nKfold):
            ...
            for ii, (g_type, g) in enumerate(zip(['undirected_weight','undirected_adjmat','directed_weight'],[UG_weight,UG_adjmat,DG_weight])):
                #Get degree distribution
                degree_dist = list(dict(g.degree()).values())
                sns.kdeplot(degree_dist,ax=axes[ii],color=cmap[jj],label=filename_list[jj])
                ...
                usrplt.visualize_matrix(FCF_optimal[jj,iK],ax=axes[0],cmap='viridis',title='Sorted by area',clims=[0,vmax_fcf],center=None,ticks=ticks,labels=labels,boundaries=boundaries)
    ...
    try:
        ...
        prs.save(join(SaveDir,f'FCF_figs_{mID}_{rec_name}.pptx'))
        exit()
